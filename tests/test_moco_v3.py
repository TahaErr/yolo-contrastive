...
class TestTrainSmoke:
    ...
    def test_resume_continues_from_checkpoint(self, tmp_path):
        ...
        MoCo-v3 resume must also restore the momentum encoder + momentum
        projector — exercised here implicitly (training stays stable
        across the interruption boundary)."""
        import shutil
        from yolo_contrastive.baselines import moco_v3 as _mv

        img_dir = _dummy_image_dir(n=4, size=64)
        out = tmp_path / "moco.pt"
        resume_path = str(out).replace(".pt", ".resume.pt")
        try:
            real_save = _mv.MoCoV3YOLOTrainer._save
            ...
            crashed = False
            try:
                _mv.MoCoV3YOLOTrainer._save = crashing_save
                try:
                ...
                _mv.MoCoV3YOLOTrainer._save = real_save
                tr1.cleanup()
